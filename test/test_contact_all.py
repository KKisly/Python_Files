...
def test_contacts_on_home_page(app):
    ...
    assert contact_from_home_page.name == merge_names_like_on_home_page(contact_from_edit_page)
    assert contact_from_home_page.last_name == merge_last_names_like_on_home_page(contact_from_edit_page)
    ...
    return "\n".join(filter(lambda x: x != "",
                     map(lambda x: clear(x),
                         filter(lambda x: x is not None,
                                [contact.telephone_home, contact.telephone_work, contact.telephone_mobile, contact.fax]))))
...
def merge_last_names_like_on_home_page(contact):
    return "\n".join(filter(lambda x: x != "",
                     map(lambda x: clear(x),
                         filter(lambda x: x is not None,
                                [contact.last_name]))))
